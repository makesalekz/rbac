...
type TeamQuery struct {
	...
	withParent   *TeamQuery
	withChildren *TeamQuery
...
func (tq *TeamQuery) QueryParent() *TeamQuery {
...
func (tq *TeamQuery) QueryChildren() *TeamQuery {
	query := (&TeamClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (tq *TeamQuery) IDs(ctx context.Context) (ids []int64, err error) {
...
func (tq *TeamQuery) WithParent(opts ...func(*TeamQuery)) *TeamQuery {
	query := (&TeamClient{config: tq.config}).Query()
...
func (tq *TeamQuery) WithChildren(opts ...func(*TeamQuery)) *TeamQuery {
	query := (&TeamClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TeamQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Team, error) {
	...
	if query := tq.withParent; query != nil {
		if err := tq.loadParent(ctx, query, nodes, nil,
			func(n *Team, e *Team) { n.Edges.Parent = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withChildren; query != nil {
		if err := tq.loadChildren(ctx, query, nodes,
			func(n *Team) { n.Edges.Children = []*Team{} },
			func(n *Team, e *Team) { n.Edges.Children = append(n.Edges.Children, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TeamQuery) loadParent(ctx context.Context, query *TeamQuery, nodes []*Team, init func(*Team), assign func(*Team, *Team)) error {
...
func (tq *TeamQuery) loadChildren(ctx context.Context, query *TeamQuery, nodes []*Team, init func(*Team), assign func(*Team, *Team)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(team.FieldParentID)
	}
	query.Where(predicate.Team(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(team.ChildrenColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (tq *TeamQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withParent != nil {
			_spec.Node.AddColumnOnce(team.FieldParentID)
		}
	}
