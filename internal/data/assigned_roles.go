...
type AssignedRolesRepo interface {
	...
	GetAssignedRolesByRoleIDs(ctx context.Context, tenantID int64, roleIDs []int64) ([]*ent.ResourceAccess, error)
}
...
func (t *assignedRolesRepo) GetAssignedRolesByRoleIDs(ctx context.Context, tenantID int64, roleIDs []int64) (
	[]*ent.ResourceAccess, error,
) {
	return t.db.ResourceAccess.Query().
		Where(
			resourceaccess.TenantID(tenantID),
			resourceaccess.RoleIDIn(roleIDs...),
		).
		All(ctx)
}
