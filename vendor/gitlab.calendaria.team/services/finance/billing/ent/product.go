...
type Product struct {
	...
	// Indicates that this product is active.
	IsActive bool `json:"is_active,omitempty"`
	// Indicates that this product is limited.
	IsLimited bool `json:"is_limited,omitempty"`
	// End of limited period.
	LimitedTill *time.Time `json:"limited_till,omitempty"`
	// Number of items left in stock.
	Left int64 `json:"left,omitempty"`
	// Indicates that this product can only be limited amount of times.
...
func (e ProductEdges) InvoicesOrErr() ([]*Invoice, error) {
	if e.loadedTypes[0] {
...
func (pr *Product) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case product.FieldIsLimited:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field is_limited", values[i])
			} else if value.Valid {
				pr.IsLimited = value.Bool
			}
		case product.FieldLimitedTill:
			if value, ok := values[i].(*sql.NullTime); !ok {
				return fmt.Errorf("unexpected type %T for field limited_till", values[i])
			} else if value.Valid {
				pr.LimitedTill = new(time.Time)
				*pr.LimitedTill = value.Time
			}
		...
		case product.FieldIsUnique:
			if value, ok := values[i].(*sql.NullBool); !ok {
				...
				pr.IsUnique = value.Bool
			}
		case product.FieldUniqueLimit:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field unique_limit", values[i])
			} else if value.Valid {
...
func (pr *Product) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("is_limited=")
	builder.WriteString(fmt.Sprintf("%v", pr.IsLimited))
	builder.WriteString(", ")
	if v := pr.LimitedTill; v != nil {
		builder.WriteString("limited_till=")
		builder.WriteString(v.Format(time.ANSIC))
	}
	builder.WriteString(", ")
	builder.WriteString("left=")
	builder.WriteString(fmt.Sprintf("%v", pr.Left))
	builder.WriteString(", ")
	builder.WriteString("is_unique=")
	builder.WriteString(fmt.Sprintf("%v", pr.IsUnique))
	builder.WriteString(", ")
