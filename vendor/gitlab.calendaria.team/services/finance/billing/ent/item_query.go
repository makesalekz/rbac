...
func (iq *ItemQuery) QueryBundles() *BundleQuery {
	query := (&BundleClient{config: iq.config}).Query()
...
func (iq *ItemQuery) WithBundles(opts ...func(*BundleQuery)) *ItemQuery {
	query := (&BundleClient{config: iq.config}).Query()
...
func (iq *ItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Item, error) {
	...
	if query := iq.withBundles; query != nil {
		if err := iq.loadBundles(ctx, query, nodes,
			func(n *Item) { n.Edges.Bundles = []*Bundle{} },
			func(n *Item, e *Bundle) { n.Edges.Bundles = append(n.Edges.Bundles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *ItemQuery) loadBundles(ctx context.Context, query *BundleQuery, nodes []*Item, init func(*Item), assign func(*Item, *Bundle)) error {
