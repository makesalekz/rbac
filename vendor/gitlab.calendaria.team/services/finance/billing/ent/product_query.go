...
func (pq *ProductQuery) QueryInvoices() *InvoiceQuery {
	query := (&InvoiceClient{config: pq.config}).Query()
...
// QueryBundles chains the current query on the "bundles" edge.
func (pq *ProductQuery) QueryBundles() *BundleQuery {
	query := (&BundleClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProductQuery) WithInvoices(opts ...func(*InvoiceQuery)) *ProductQuery {
	query := (&InvoiceClient{config: pq.config}).Query()
...
// WithBundles tells the query-builder to eager-load the nodes that are connected to
// the "bundles" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProductQuery) WithBundles(opts ...func(*BundleQuery)) *ProductQuery {
	query := (&BundleClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if query := pq.withInvoices; query != nil {
		if err := pq.loadInvoices(ctx, query, nodes,
			func(n *Product) { n.Edges.Invoices = []*Invoice{} },
			func(n *Product, e *Invoice) { n.Edges.Invoices = append(n.Edges.Invoices, e) }); err != nil {
			return nil, err
		}
	}
...
func (pq *ProductQuery) loadInvoices(ctx context.Context, query *InvoiceQuery, nodes []*Product, init func(*Product), assign func(*Product, *Invoice)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(invoice.FieldProductID)
	}
...
func (pq *ProductQuery) loadBundles(ctx context.Context, query *BundleQuery, nodes []*Product, init func(*Product), assign func(*Product, *Bundle)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Product)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(bundle.FieldProductID)
	}
	query.Where(predicate.Bundle(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.BundlesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
